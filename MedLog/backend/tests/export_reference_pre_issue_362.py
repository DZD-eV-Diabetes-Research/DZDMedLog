"""Frozen copy of the study exporter as it was before issue #362.

Only used by `tests_export_performance.py` as the reference for the regression
test: the rewritten exporter must produce byte-identical CSV and JSON files. The
benchmark also runs it to show the before/after numbers. Do not "fix" or
optimize this file, it is supposed to stay slow.
"""
