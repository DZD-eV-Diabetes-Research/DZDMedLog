...
from medlogserver.model._base_model import TimestampModel, BaseTable
...
class _UserAuthBase(BaseTable, table=False):
    ...
    auth_source_type: AllowedAuthSchemeType = Field(
        ...
        sa_column=Column(Enum(AllowedAuthSchemeType, name="allowedauthschemetype")),
    )
...
class UserAuth(_UserAuthBase, TimestampModel, table=True):
    ...
    def update_oidc_token(self, oidc_token: Dict):
        ...
        Args:
            oidc_token (Dict): example `{'access_token': 'pnAkhmYYMwyApZCRhDKx41Mvb3daCrOOAlqbICHhzm', 'expires_in': 3600, 'scope': 'openid profile email offline-access', 'token_type': 'Bearer', 'expires_at': 1747088292}`
        """
