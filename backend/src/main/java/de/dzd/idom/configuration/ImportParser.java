...
import de.dzd.idom.drug.DrugService;
import de.dzd.idom.drug.association.*;
import de.dzd.idom.util.FileCompressor;
...
public class ImportParser {
    ...
    private void createAssociations() {
        ...
        Instant end = Instant.now();
        String duration = DurationFormatUtils.formatDurationHMS(Duration.between(start, end).toMillis());
        int size = atcMap.size() + dosageFormMap.size() + applicationFormMap.size() + normPackageSizeMap.size() + producerMap.size();
    ...
    private Map<String, ApplicationForm> createApplicationForms() {
        ...
        return applicationForms.stream().collect(Collectors.toMap(ApplicationForm::getAbbreviation, Function.identity()));
    }
